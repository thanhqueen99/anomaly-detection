...
# Load unpropagated Sentinel 3A dataset into R
sentinel3a <- read_csv('/Users/vuminh/Documents/satellite_data/orbital_elements/unpropagated_elements/unpropagated_elements_Sentinel-3A.csv')
...
# EDA plots for Sentinel 3A features
for (ft in fts) {
  eda(sentinel3a, ft)
}
...
# POSIX timestamps
sentinel3a$timestamp <- as.POSIXct(sentinel3a$timestamp, format="%Y-%m-%d %H:%M:%S")

# Manoeuvres for Sentinel 3A
...
predsat <- function(df, feat, upth, lwth) {
  ...
  pred_obs <- ggplot(respl, aes(x = timestamp, y = resi)) +
    ...
    labs(
      title = "Sentinel 3A Manoeuvres predicted versus observed",
      x = "Years",
      y = "Residuals"
    ) +
    theme_minimal()
