...
# Load unpropagated Haiyang 2A dataset into R
haiyang2a <- read_csv('/Users/vuminh/Documents/satellite_data/orbital_elements/unpropagated_elements/unpropagated_elements_Haiyang-2A.csv')
...
# EDA plots for Haiyang 2A features
for (ft in fts) {
  eda(haiyang2a, ft)
}
...
# POSIX timestamps
haiyang2a$timestamp <- as.POSIXct(haiyang2a$timestamp, format="%Y-%m-%d %H:%M:%S")

# Manoeuvres for Haiyang 2A
...
predsat <- function(df, feat, upth, lwth) {
  ...
  pred_obs <- ggplot(respl, aes(x = timestamp, y = resi)) +
    ...
    labs(
      title = "Haiyang 2A Manoeuvres predicted versus observed",
      x = "Years",
      y = "Residuals"
    ) +
    theme_minimal()
...
satdt[["Haiyang-2A"]] <- list(unpr = haiyang2a)

resplot <- function(satdt, feat, upth, lwth) {
  dt <- satdt[["Haiyang-2A"]]$unpr
