...
topex <- read_csv(
  '/Users/vuminh/Documents/satellite_data/orbital_elements/unpropagated_elements/unpropagated_elements_TOPEX.csv')
...
# Centre and scale eccentricity
topex <- topex %>%
  mutate(`eccentricity` = scale(`eccentricity`))
head(topex$`eccentricity`)
...
satdt <- list(TOPEX = list(unpr = topex))
...
plotprerec <- function(prerec, model, satname, feat) {
  ggplot(prerec, aes(x = recall, y = precision)) +
    ...
    labs(
      title = "Precision-Recall of ARIMA Predicting eccentricity Residuals on TOPEX",
      x = "Recall",
      y = "Precision"
    ) +
    theme_classic() +  
...
# ARIMA function
arimaft <- function(TOPEX, feat, satdt, thrd = 0.763) {
  ...
  arimaplot <- ggplot(testset, 
    ...
    geom_vline(aes(xintercept = timestamp),
               ...
               linetype = "dashed") + labs(
                 ...
                 title = glue::glue("ARIMA: {TOPEX} Manoeuvres Observed versus Predicted on {feat}"),
                 x = "Time",
                 y = "Residuals"
               ) +
    theme_classic() +  
...
arimametrics <- function(satdt, feat, thrd = 0.763) {
  # Anomaly detection with ARIMA 
  aricall <- arimaft(
    TOPEX = "TOPEX",
    feat = feat,
...
arimametrics(satdt, "eccentricity", thrd = 0.763)
...
predsat <- function(df, feat, upth, lwth) {
  ...
  pred_obs <- ggplot(respl, aes(x = timestamp, y = abs(resi))) +
    ...
    labs(subtitle = "Observed Manoeuvres: Green solid lines, Predicted Manoeuvres: Blue dashed lines",
         title = "XGBoost: TOPEX Manoeuvres Observed versus Predicted on eccentricity",
         x = "Time",
         y = "Residuals"
    ) +
    theme_classic() +  
...
plotprerec <- function(prerec) {
  ggplot(prerec, 
    ...
    geom_line(color = "blue") +
    labs(
      title = "Precision-Recall of XGBoost Predicting eccentricity Residuals on TOPEX", 
...
# Run function
resplot(satdt, "eccentricity", upth = 1, lwth = -1)
