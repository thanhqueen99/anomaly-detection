...
saral <- read_csv(
  '/Users/vuminh/Documents/satellite_data/orbital_elements/unpropagated_elements/unpropagated_elements_SARAL.csv')
...
eda <- function(df, ft) {
  ...
  lin <- ggplot(df, aes(x = timestamp, y = .data[[ft]])) +
    ...
    xlab('Years') +
    ylab(ft) +
    ggtitle(paste('Line graph of', ft, 'over years for SARAL')) +
    theme_bw() + 
  ...
  hst <- ggplot(df, aes(x = .data[[ft]])) +
    ...
    ggtitle(paste('Histogram of', ft, 'distribution for SARAL')) +
    theme_bw() + 
    theme(plot.title = element_text(size = 14),
          axis.title.x = element_text(size = 14),
          axis.title.y = element_text(size = 14),
  ...
  bxp <- ggplot(df, aes(y = .data[[ft]])) +
    ...
    ylab(ft) +
    ggtitle(paste('Box plot of', ft, 'for SARAL')) +
    theme_bw() + 
    theme(plot.title = element_text(size = 14),
          axis.title.x = element_text(size = 14),
          axis.title.y = element_text(size = 14),
...
head(saral$`Brouwer mean motion`)
...
# POSIX timestamps
saral$timestamp <- as.POSIXct(
  saral$timestamp, 
  format="%Y-%m-%d %H:%M:%S")

# Update satellite with Manoeuvre data 
fp <- '/Users/vuminh/Documents/satellite_data/manoeuvres/srlman.txt'
saral <- upd(saral, fp)

satdt <- list(SARAL = list(unpr = saral))
...
plotprerec <- function(prerec, model, satname, feat) {
  ggplot(prerec, aes(x = recall, y = precision)) +
    ...
    labs(
      title = "Precision-Recall of ARIMA Predicting Brouwer mean motion Residuals on SARAL",
      x = "Recall",
      y = "Precision"
    ) +
    theme_classic() +  
...
# ARIMA function
arimaft <- function(saral, feat, satdt, thrd = 0.00729) {
  ...
  arimaplot <- ggplot(testset, 
    ...
    geom_vline(aes(xintercept = timestamp),
               ...
               linetype = "dashed") + labs(
                 ...
                 title = glue::glue("ARIMA: {saral} Manoeuvres Observed versus Predicted on {feat}"),
                 x = "Time",
                 y = "Residuals"
               ) +
    theme_classic() + 
...
arimametrics <- function(satdt, feat, thrd = 0.00729) {
  # Anomaly detection with ARIMA 
  aricall <- arimaft(
    saral = "SARAL",
    feat = feat,
...
arimametrics(satdt, "Brouwer mean motion", thrd = 0.00729)
...
predsat <- function(df, feat, upth, lwth) {
  ...
  pred_obs <- ggplot(respl, aes(x = timestamp, y = abs(resi))) +
    ...
    labs(subtitle = "Observed Manoeuvres: Green solid lines, Predicted Manoeuvres: Blue dashed lines",
         title = "XGBoost: SARAL Manoeuvres Observed versus Predicted on Brouwer mean motion",
         x = "Time",
         y = "Residuals"
    ) +
    theme_classic() + 
...
plotprerec <- function(prerec) {
  ggplot(prerec, 
    ...
    geom_line(color = "blue") +
    labs(
      title = "Precision-Recall of XGBoost Predicting Brouwer mean motion Residuals on SARAL", 
