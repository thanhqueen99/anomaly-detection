...
# Load unpropagated CryoSat 2 dataset into R
cryosat2 <- read_csv('/Users/vuminh/Documents/satellite_data/orbital_elements/unpropagated_elements/unpropagated_elements_CryoSat-2.csv')
...
# EDA plots for CryoSat 2 features
for (ft in fts) {
  eda(cryosat2, ft)
}
...
# POSIX timestamps
cryosat2$timestamp <- as.POSIXct(cryosat2$timestamp, format="%Y-%m-%d %H:%M:%S")

# Manoeuvres for CryoSat 2
...
predsat <- function(df, feat, upth, lwth) {
  ...
  pred_obs <- ggplot(respl, aes(x = timestamp, y = resi)) +
    ...
    labs(
      title = "CryoSat 2 Manoeuvres predicted versus observed",
      x = "Years",
      y = "Residuals"
    ) +
    theme_minimal()
...
satdt[["CryoSat-2"]] <- list(unpr = cryosat2)

resplot <- function(satdt, feat, upth, lwth) {
  dt <- satdt[["CryoSat-2"]]$unpr
