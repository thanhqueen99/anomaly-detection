...
haiyang2a <- read_csv(
  '/Users/vuminh/Documents/satellite_data/orbital_elements/unpropagated_elements/unpropagated_elements_Haiyang-2A.csv')
...
eda <- function(df, ft) {
  ...
  lin <- ggplot(df, aes(x = timestamp, y = .data[[ft]])) +
    ...
    xlab('Years') +
    ylab(ft) +
    ggtitle(paste('Line graph of', ft, 'over years for Haiyang 2A')) +
    theme_bw() + 
  ...
  hst <- ggplot(df, aes(x = .data[[ft]])) +
    ...
    ggtitle(paste('Histogram of', ft, 'distribution for Haiyang 2A')) +
    theme_bw() + 
    theme(plot.title = element_text(size = 14),
          axis.title.x = element_text(size = 14),
          axis.title.y = element_text(size = 14),
  ...
  bxp <- ggplot(df, aes(y = .data[[ft]])) +
    ...
    ylab(ft) +
    ggtitle(paste('Box plot of', ft, 'for Haiyang 2A')) +
    theme_bw() + 
    theme(plot.title = element_text(size = 14),
          axis.title.x = element_text(size = 14),
          axis.title.y = element_text(size = 14),
...
fts <- colnames(haiyang2a)[2:7]
for (ft in fts) {
  eda(haiyang2a, ft)
}

# Centre and scale argument of perigee
haiyang2a <- haiyang2a %>%
  mutate(`argument of perigee` = scale(`argument of perigee`))
head(haiyang2a$`argument of perigee`)
...
# POSIX timestamps
haiyang2a$timestamp <- as.POSIXct(
  haiyang2a$timestamp, 
  format="%Y-%m-%d %H:%M:%S")

# Update satellite with Manoeuvre data 
fp <- '/Users/vuminh/Documents/satellite_data/manoeuvres/h2aman.txt'
haiyang2a <- upd(haiyang2a, fp)

satdt <- list(Haiyang_2A = list(unpr = haiyang2a))
...
plotprerec <- function(prerec, model, satname, feat) {
  ggplot(prerec, aes(x = recall, y = precision)) +
    ...
    labs(
      title = "Precision-Recall of ARIMA Predicting argument of perigee Residuals on Haiyang 2A",
      x = "Recall",
      y = "Precision"
    ) +
    theme_classic() + 
...
# ARIMA function
arimaft <- function(Haiyang_2A, feat, satdt, thrd = 0.253) {
  ...
  arimaplot <- ggplot(testset, 
    ...
    geom_vline(aes(xintercept = timestamp),
               ...
               linetype = "dashed") + labs(
                 ...
                 title = glue::glue("ARIMA: {Haiyang_2A} Manoeuvres Observed versus Predicted on {feat}"),
                 x = "Time",
                 y = "Residuals"
               ) +
    theme_classic() + 
...
aricall <- arimaft(
  Haiyang_2A = "Haiyang_2A",
  feat = "argument of perigee",
  satdt = satdt,
  thrd = 0.253
)

# ARIMA Precision, Recall, F-score
arimametrics <- function(satdt, feat, thrd = 0.253) {
  # Anomaly detection with ARIMA 
  aricall <- arimaft(
    Haiyang_2A = "Haiyang_2A",
    feat = feat,
...
arimametrics(satdt, "argument of perigee", thrd = 0.253)
...
predsat <- function(df, feat, upth, lwth) {
  ...
  pred_obs <- ggplot(respl, aes(x = timestamp, y = abs(resi))) +
    ...
    labs(subtitle = "Observed Manoeuvres: Green solid lines, Predicted Manoeuvres: Blue dashed lines",
         title = "XGBoost: Haiyang 2A Manoeuvres Observed versus Predicted on argument of perigee",
         x = "Time",
         y = "Residuals"
    ) +
    theme_classic() + 
...
plotprerec <- function(prerec) {
  ggplot(prerec, 
    ...
    geom_line(color = "blue") +
    labs(
      title = "Precision-Recall of XGBoost Predicting argument of perigee Residuals on Haiyang 2A", 
...
satdt <- list()  
satdt[["Haiyang_2A"]] <- list(unpr = haiyang2a)

# Run function
resplot(satdt, "argument of perigee", upth = 1, lwth = -1)
