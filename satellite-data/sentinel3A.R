...
sentinel3a <- read_csv(
  '/Users/vuminh/Documents/satellite_data/orbital_elements/unpropagated_elements/unpropagated_elements_Sentinel-3A.csv')
...
eda <- function(df, ft) {
  ...
  lin <- ggplot(df, aes(x = timestamp, y = .data[[ft]])) +
    ...
    xlab('Years') +
    ylab(ft) +
    ggtitle(paste('Line graph of', ft, 'over years for Sentinel 3A')) +
    theme_bw() + 
  ...
  hst <- ggplot(df, aes(x = .data[[ft]])) +
    ...
    ggtitle(paste('Histogram of', ft, 'distribution for Sentinel 3A')) +
    theme_bw() + 
    theme(plot.title = element_text(size = 14),
          axis.title.x = element_text(size = 14),
          axis.title.y = element_text(size = 14),
  ...
  bxp <- ggplot(df, aes(y = .data[[ft]])) +
    ...
    ylab(ft) +
    ggtitle(paste('Box plot of', ft, 'for Sentinel 3A')) +
    theme_bw() + 
    theme(plot.title = element_text(size = 14),
          axis.title.x = element_text(size = 14),
          axis.title.y = element_text(size = 14),
...
head(sentinel3a$`Brouwer mean motion`)
...
# POSIX timestamps
sentinel3a$timestamp <- as.POSIXct(
  sentinel3a$timestamp, 
  format="%Y-%m-%d %H:%M:%S")

# Update satellite with Manoeuvre data 
fp <- '/Users/vuminh/Documents/satellite_data/manoeuvres/s3aman.txt'
sentinel3a <- upd(sentinel3a, fp)

satdt <- list(Sentinel_3A = list(unpr = sentinel3a))
...
plotprerec <- function(prerec, model, satname, feat) {
  ggplot(prerec, aes(x = recall, y = precision)) +
    ...
    labs(
      title = "Precision-Recall of ARIMA Predicting Brouwer mean motion Residuals on Sentinel 3A",
      x = "Recall",
      y = "Precision"
    ) +
    theme_classic() + 
...
# ARIMA function
arimaft <- function(sentinel_3a, feat, satdt, thrd = 0.255) {
  ...
  arimaplot <- ggplot(testset, 
    ...
    geom_vline(aes(xintercept = timestamp),
               ...
               linetype = "dashed") + labs(
                 ...
                 title = glue::glue("ARIMA: {sentinel_3a} Manoeuvres Observed versus Predicted on {feat}"),
                 x = "Time",
                 y = "Residuals"
               ) +
    theme_classic() + 
...
arimametrics <- function(satdt, feat, thrd = 0.255) {
  # Anomaly detection with ARIMA 
  aricall <- arimaft(
    sentinel_3a = "Sentinel_3A",
    feat = feat,
...
arimametrics(satdt, "Brouwer mean motion", thrd = 0.255)
...
predsat <- function(df, feat, upth, lwth) {
  ...
  pred_obs <- ggplot(respl, aes(x = timestamp, y = abs(resi))) +
    ...
    labs(subtitle = "Observed Manoeuvres: Green solid lines, Predicted Manoeuvres: Blue dashed lines",
         title = "XGBoost: Sentinel 3A Manoeuvres Observed versus Predicted on Brouwer mean motion",
         x = "Time",
         y = "Residuals"
    ) +
    theme_classic() + 
...
plotprerec <- function(prerec) {
  ggplot(prerec, 
    ...
    geom_line(color = "blue") +
    labs(
      title = "Precision-Recall of XGBoost Predicting Brouwer mean motion Residuals on Sentinel 3A", 
...
satdt <- list()  
satdt[["Sentinel_3A"]] <- list(unpr = sentinel3a)
