...
# Load unpropagated SARAL dataset into R
saral <- read_csv('/Users/vuminh/Documents/satellite_data/orbital_elements/unpropagated_elements/unpropagated_elements_SARAL.csv')
...
# POSIX timestamps
saral$timestamp <- as.POSIXct(saral$timestamp, format="%Y-%m-%d %H:%M:%S")

# Manoeuvres for SARAL
...
predsat <- function(df, feat, upth, lwth) {
  ...
  pred_obs <- ggplot(respl, aes(x = timestamp, y = resi)) +
    ...
    labs(
      title = "SARAL Manoeuvres predicted versus observed",
      x = "Years",
      y = "Residuals"
    ) +
    theme_minimal()
